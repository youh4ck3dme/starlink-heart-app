...
describe('MascotRenderer', () => {
  ...
  it('renders Image when mode is image', () => {
    ...
    expect(screen.getByAltText('Starry')).toBeInTheDocument();
  });
  ...
  it('FALLBACKS to Rive when mode is spline3d BUT scene is NOT configured', () => {
    ...
    // Should see Rive (default fallback)
    expect(screen.getByTestId('rive-mascot')).toBeInTheDocument();
  });
});
