...
import cometAvatar from '../../assets/avatars/cometa.png';
  ...
  return (
    ...
    >
      ...
      >
        ...
        {/* Planet Core */}
        <div className={`relative w-28 h-28 sm:w-36 sm:h-36 rounded-full border-4 shadow-2xl overflow-hidden backdrop-blur-sm bg-black/40 flex items-center justify-center`}
        ...
        <div className="absolute -bottom-10 left-1/2 -translate-x-1/2 w-max text-center">
            ...
            <span className="text-xs font-bold text-white/70 bg-black/30 px-2 py-0.5 rounded-full backdrop-blur-md">
              LVL {levelRange}
            </span>
        </div>
      </motion.button>
...
export default function GalaxyRoadmap() {
  ...
  const handleStartMission = (planet: string) => {
    haptics.mediumTap();
    // Simulate warp speed effect or just navigate
    // For now we just check if it's the first available level
    localStorage.setItem('hasStarted', 'true');
    navigate('/home');
  };

  return (
    <div ref={containerRef} className="relative w-full max-w-lg mx-auto pb-32 pt-10 px-4">
      
      {/* Connecting Path (SVG) */}
      <div className="absolute top-20 bottom-32 left-1/2 -translate-x-1/2 w-full max-w-[300px] pointer-events-none -z-10 h-full">
         <svg className="w-full h-full overflow-visible" preserveAspectRatio="none">
             {/* Defined curve path logic would go here, simplistic dashed line for now */}
             <motion.path
                d="M 150 50 C 50 200, 250 400, 150 600 C 50 800, 250 1000, 150 1150" 
                fill="none"
      ...
      >
        <button 
          ...
          className="text-white/50 text-sm font-medium hover:text-white transition-colors uppercase tracking-widest border-b border-transparent hover:border-white/50"
        >
          Preskočiť intro ⏩
        </button>
      </motion.div>
